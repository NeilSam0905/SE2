...
function Dashboard({ onLogout, onNavigate }) {
  ...
  const getFilteredOrders = () => {
    ...
    return orders.filter(order => {
      const orderDate = new Date(order.orderTimestamp || order.createdAt)
  ...
  const getFilteredPayments = () => {
    ...
    return payments.filter(payment => {
      const paymentDate = new Date(payment.paymentTimestamp || payment.createdAt)
  ...
  // Calculate total sales from filtered payments
  const calculateTotalSales = () => {
    const total = filteredPayments.reduce((sum, payment) => sum + (payment.totalAmount || 0), 0)
    console.log('Filtered Payments:', filteredPayments)
    console.log('Total Sales Calculated:', total)
    return total
  }

  // Calculate dine-in and take-out ratio
  const calculateDineInTakeOutRatio = () => {
    if (filteredOrders.length === 0) {
      return { dineInPct: 0, takeOutPct: 0 }
    }

    const dineInCount = filteredOrders.filter(order => order.orderType === 'Dine-In').length
    const takeOutCount = filteredOrders.filter(order => order.orderType === 'Take-Out').length
    const totalOrders = filteredOrders.length

    const dineInPct = totalOrders > 0 ? Math.round((dineInCount / totalOrders) * 100) : 0
    const takeOutPct = 100 - dineInPct

    return { dineInPct, takeOutPct }
  }
  ...
  const getStatsData = () => {
    ...
    const totalSales = calculateTotalSales().toLocaleString('en-PH')
    const { dineInPct, takeOutPct } = calculateDineInTakeOutRatio()
    switch (timeFilter) {
      case 'Daily':
        return {
          revenue: totalSales,
